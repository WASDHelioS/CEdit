...
public class CEditHandler extends CommandHandler implements CommandExecutor {
    ...
    public List<String> getCommandArgs(String[] arguments) {
        ...
        for (int i = 0; i < returnList.size(); i++) {
            ...
            returnList.set(i, returnList.get(i).replaceAll("\\s+(?=&)", ""));
            returnList.set(i, returnList.get(i).replaceAll("&+", "&"));
        }
    ...
    private void sendCEditHelpMessage(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.GOLD + "/CEdit add /<to command> /<from command> : Adds a command.");
        sender.sendMessage(ChatColor.GOLD + "/CEdit add /<to command> /<from command> & /<from command> etc : adds multiple commands to one.");
        sender.sendMessage(ChatColor.GOLD + "/CEdit edit index <index> /<new to command> /<new from command> : edits the command at the specified index (retrievable by list)(Allows for multiple commands the same way add does.)");
        sender.sendMessage(ChatColor.GOLD + "/CEdit remap /<existing tocommand> /<new fromcommand> : Maps the existing tocommand to a new fromcommand. (Replaces the old one.)(Allows for multiple commands the same way add does.)");
